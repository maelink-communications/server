...
import { emitHomeDiscussion, emitHomePost, emitHomePostEdit, emitHomePostDelete, emitHomePostLike } from "./socket.js";
...
export async function createComment(token, postId, content) {
  ...
  db.prepare(`UPDATE posts SET comment_count = comment_count + 1 WHERE uuid = ?`)
    .run(post.uuid);
...
export async function fetchComments(token, postId) {
  ...
  ).all(post.uuid).map(publicDiscussion);
}
...
export async function deleteComment(token, postId, commentId) {
  ...
  if (!post || !comment || comment.post_id !== post.uuid || comment.user_id !== userId) {
    return false;
  }
  ...
  emitHomeDiscussion("comment:delete", post.uuid, { id: comment.id, uuid: comment.uuid });
  return true;
}

export async function createReply(token, postId, content, parentReplyId = null) {
  const userId = await resolveUser(token);
  const post = resolvePost(postId);
  const normalized = discussionContent(content);
  if (!post || !normalized) return false;
  ...
  db.prepare(`UPDATE posts SET reply_count = reply_count + 1 WHERE uuid = ?`)
    .run(post.uuid);
...
export async function deleteReply(token, postId, replyId) {
  ...
  if (!post || !reply || reply.post_id !== post.uuid || reply.user_id !== userId) {
    return false;
  }
  ...
  emitHomeDiscussion("reply:delete", post.uuid, { id: reply.id, uuid: reply.uuid });
  return true;
}
